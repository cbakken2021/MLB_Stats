...
LoadData<- function() {
  ...
  Clean3 <- subset(Clean3, select = -c(playerID, throws, stint, CS, IBB, HBP, SH, SF, GIDP, G, nameGiven))
  return(Clean3)
}
...
LoadTeamNames<- function() {
  df<- LoadData()
  return(unique(df$teamID))
}

LoadColNames<- function() {
  df<- LoadData()
  df<- subset(df, select = -c(FullName))
  cnames<-colnames(df)
  newcolnames <- c('Choices', cnames)
  return(newcolnames)
